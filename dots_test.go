...
func TestDotsPublicReferences(t *testing.T) {
	data, err := os.ReadFile("testdata/dots_mathjax_3_2_2.json")
	if err != nil {
		t.Fatal(err)
	}
	var fixture struct {
	...
	if fixture.MathjaxGitCommit != "ad8f5c21cb810236551da8c6512ba733e67357ee" || len(fixture.Cases) != 48 {
		t.Fatal("unbound dots references")
	}
	seen := make(map[string]bool)
	for _, c := range fixture.Cases {
		if c.Name == "" || seen[c.Name] || c.SVG == "" || c.Width <= 0 || c.Height <= 0 {
			t.Fatal("invalid dots reference inventory", c.Name)
		}
		seen[c.Name] = true
