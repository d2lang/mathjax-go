...
func TestEquationNestingPublicReferences(t *testing.T) {
	var fixture struct {
	...
	data, err := os.ReadFile("testdata/equation_nesting_mathjax_3_2_2.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	if fixture.MathjaxGitCommit != "ad8f5c21cb810236551da8c6512ba733e67357ee" || len(fixture.Cases) != 8 {
		t.Fatal("unbound equation-nesting references")
	}
	targets := 0
	for _, c := range fixture.Cases {
		...
		t.Run(c.Name, func(t *testing.T) {
			...
			if c.Target {
				// Measure uses the display renderer for both retained modes.
	...
	if targets != 4 {
		t.Fatal("changed rendered-error measurement inventory")
	}
}
