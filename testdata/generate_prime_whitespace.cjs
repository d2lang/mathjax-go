...
// D067 complete SVG and observed tree fixture generator.
...
if (!base)
  throw Error(
    "usage: node generate_prime_whitespace.cjs /path/to/pinned/d2latex [evidence-directory]"
  );
...
const inputs = JSON.parse(fs.readFileSync(path.join(__dirname, 'prime_whitespace_inputs.json')));
...
fs.writeFileSync(path.join(evidence||__dirname,'prime_whitespace_mathjax_3_2_2.json'),JSON.stringify({oracle:'Unmodified pinned D2 MathJax3.2.2; fresh VM per case/mode; AST observed before unchanged SVG renderer',mathjaxGitCommit:'ad8f5c21cb810236551da8c6512ba733e67357ee',assetsSHA256:hashes,cases:records},null,2)+'\n');console.log(records.filter(r=>!r.runtimeError).length+' complete primary SVG/tree references; '+records.filter(r=>r.runtimeError).length+' primary runtime errors without output');
