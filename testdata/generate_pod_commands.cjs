...
// Source-only D068 pod/pmod reference inventory with separate mod/bmod controls.
...
if (!base)
  throw Error(
    "usage: node generate_pod_commands.cjs /path/to/pinned/d2latex [evidence-directory]"
  );
...
const inputs = [["pod-plain", "\\pod{x}", false], ["pod-plain", "\\pod{x}", true], ["pmod-plain", "\\pmod{x}", false], ["pmod-plain", "\\pmod{x}", true], ["pod-binrow", "x+\\pod{n}+y", false], ["pod-binrow", "x+\\pod{n}+y", true], ["pmod-relrow", "x= y\\pmod{n}", false], ["pmod-relrow", "x= y\\pmod{n}", true], ["pod-displaystyle", "\\displaystyle\\pod{x}", false], ["pod-displaystyle", "\\displaystyle\\pod{x}", true], ["pmod-displaystyle", "\\displaystyle\\pmod{x}", false], ["pmod-displaystyle", "\\displaystyle\\pmod{x}", true], ["pod-textstyle", "\\textstyle\\pod{x}", false], ["pod-textstyle", "\\textstyle\\pod{x}", true], ["pmod-textstyle", "\\textstyle\\pmod{x}", false], ["pmod-textstyle", "\\textstyle\\pmod{x}", true], ["pod-scriptstyle", "\\scriptstyle\\pod{x}", false], ["pod-scriptstyle", "\\scriptstyle\\pod{x}", true], ["pmod-scriptstyle", "\\scriptstyle\\pmod{x}", false], ["pmod-scriptstyle", "\\scriptstyle\\pmod{x}", true], ["pod-scriptscriptstyle", "\\scriptscriptstyle\\pod{x}", false], ["pod-scriptscriptstyle", "\\scriptscriptstyle\\pod{x}", true], ["pmod-scriptscriptstyle", "\\scriptscriptstyle\\pmod{x}", false], ["pmod-scriptscriptstyle", "\\scriptscriptstyle\\pmod{x}", true], ["pod-sup", "x^{\\pod{n}}", false], ["pod-sup", "x^{\\pod{n}}", true], ["pmod-sub", "x_{\\pmod{n}}", false], ["pmod-sub", "x_{\\pmod{n}}", true], ["pod-nested-script", "x^{y_{\\pod{n}}}", false], ["pod-nested-script", "x^{y_{\\pod{n}}}", true], ["pmod-nested-script", "x_{y^{\\pmod{n}}}", false], ["pmod-nested-script", "x_{y^{\\pmod{n}}}", true], ["pod-empty", "\\pod{}", false], ["pod-empty", "\\pod{}", true], ["pmod-empty", "\\pmod{}", false], ["pmod-empty", "\\pmod{}", true], ["pod-group", "\\pod{{a+b}}", false], ["pod-group", "\\pod{{a+b}}", true], ["pmod-group", "\\pmod{{a+b}}", false], ["pmod-group", "\\pmod{{a+b}}", true], ["pod-fraction", "\\pod{\\frac{a}{b}}", false], ["pod-fraction", "\\pod{\\frac{a}{b}}", true], ["pmod-fraction", "\\pmod{\\frac{a}{b}}", false], ["pmod-fraction", "\\pmod{\\frac{a}{b}}", true], ["pod-nested", "\\pod{a+\\pod{b}}", false], ["pod-nested", "\\pod{a+\\pod{b}}", true], ["pmod-nested", "\\pmod{a+\\pmod{b}}", false], ["pmod-nested", "\\pmod{a+\\pmod{b}}", true], ["pod-missing", "\\pod", false], ["pod-missing", "\\pod", true], ["pmod-missing", "\\pmod", false], ["pmod-missing", "\\pmod", true], ["mod-plain", "\\mod{x}", false], ["mod-plain", "\\mod{x}", true], ["bmod-plain", "\\bmod", false], ["bmod-plain", "\\bmod", true], ["mod-row", "x\\mod n", false], ["mod-row", "x\\mod n", true], ["bmod-row", "x\\bmod n", false], ["bmod-row", "x\\bmod n", true], ["fixed-choose", "{a\\choose b}", false], ["fixed-choose", "{a\\choose b}", true], ["ordinary-parentheses", "(x+y)", false], ["ordinary-parentheses", "(x+y)", true]];
...
fs.writeFileSync(path.join(evidence||__dirname,'pod_commands_mathjax_3_2_2.json'),JSON.stringify({oracle:'Unmodified pinned D2 MathJax3.2.2; fresh VM per case/mode; AST observed before unchanged SVG renderer',mathjaxGitCommit:'ad8f5c21cb810236551da8c6512ba733e67357ee',assetsSHA256:hashes,cases:records},null,2)+'\n');console.log(records.length+' exact AST and complete SVG cases');
