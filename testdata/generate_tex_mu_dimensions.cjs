...
// Source-only D077 TeX mu references; scanner/consumer boundaries remain explicit.
...
if (!base)
  throw Error(
    "usage: node generate_tex_mu_dimensions.cjs /path/to/pinned/d2latex [evidence-directory]"
  );
...
for(const [label,tex,display,directMathMLWidth] of inputs){
 ...
 records.push({name,tex,display,directMathMLWidth,width:Math.ceil(Number(result.svg.match(/width="([\d.]+)ex"/)[1])*8),height:Math.ceil(Number(result.svg.match(/height="([\d.]+)ex"/)[1])*8),tree:explicit(result.tree),propertiesTree:result.tree,svgSHA256:hash(result.svg)});
}
fs.writeFileSync(path.join(evidence||__dirname,'tex_mu_dimensions_mathjax_3_2_2.json'),JSON.stringify({oracle:'Unmodified pinned D2 MathJax3.2.2; fresh VM per case/mode; AST observed before unchanged SVG renderer',mathjaxGitCommit:'ad8f5c21cb810236551da8c6512ba733e67357ee',assetsSHA256:hashes,cases:records},null,2)+'\n');console.log(records.length+' exact AST and complete SVG cases');
