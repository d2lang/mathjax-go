...
// Direct Overunderset normalization; unchanged dispatcher boundaries stay explicit.
...
if (!base || !evidence)
  throw Error(
    "usage: node generate_overunderset_direct_base.cjs /path/to/pinned/d2latex evidence-directory"
  );
...
const inputs = [
  ...
  [
    "product",
    "\\overunderset{a}{b}{\\prod}"
  ],
  [
    "integral",
    "\\overunderset{a}{b}{\\int}"
  ],
  [
    "mathop",
    "\\overunderset{a}{b}{\\mathop{x}}"
  ],
  [
    "sum-false",
    "\\overunderset{a}{b}{\\mmlToken{mo}[movablelimits=false]{∑}}"
  ],
  [
    "sum-true",
    "\\overunderset{a}{b}{\\mmlToken{mo}[movablelimits=true]{∑}}"
  ],
  [
    "integral-false",
    "\\overunderset{a}{b}{\\mmlToken{mo}[movablelimits=false]{∫}}"
...
fs.writeFileSync(path.join(evidence,'primary.json'),JSON.stringify({oracle:'Unmodified pinned D2 MathJax3.2.2; fresh VM per case/mode; AST observed before unchanged SVG renderer',mathjaxGitCommit:'ad8f5c21cb810236551da8c6512ba733e67357ee',assetsSHA256:hashes,cases:records},null,2)+'\n');console.log(records.length+' exact AST and complete SVG cases');
