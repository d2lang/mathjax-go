...
// D071 complete SVG and observed tree fixture generator.
...
const inputs = JSON.parse(fs.readFileSync(path.join(__dirname, 'decoration_permission_mathjax_3_2_2.json'))).cases.map(({name,tex,display})=>({name,tex,display}));
