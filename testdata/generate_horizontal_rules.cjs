...
// Horizontal rule characters, nested/script contexts, and unchanged accent controls.
...
if (!base)
  throw Error(
    "usage: node generate_horizontal_rules.cjs /path/to/pinned/d2latex [evidence-directory]"
  );
...
fs.writeFileSync(path.join(__dirname,'horizontal_rule_mathjax_3_2_2.json'),JSON.stringify({oracle:'Unmodified pinned D2 MathJax3.2.2; fresh VM per case/mode; AST observed before unchanged SVG renderer',mathjaxGitCommit:'ad8f5c21cb810236551da8c6512ba733e67357ee',assetsSHA256:hashes,cases:records},null,2)+'\n');console.log(records.length+' complete SVG cases');
