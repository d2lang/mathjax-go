...
// Private Subscript/Superscript family and ownership contract; no source overlays.
...
if (!base)
  throw Error(
    "usage: node generate_script_attachment.cjs /path/to/pinned/d2latex [evidence-directory]"
  );
...
const result=vm.runInContext(fs.readFileSync(path.join(__dirname,'script_attachment_body.js'),'utf8'),c);
fs.writeFileSync(path.join(evidence,'script_attachment_mathjax_3_2_2.json'),JSON.stringify({assetsSHA256:hashes,...result},null,2)+'\n');
console.log(result.rows.length+' pinned direct-method records');
