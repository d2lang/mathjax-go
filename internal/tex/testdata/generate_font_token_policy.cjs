...
// D041 font policy references. D042 renderer and preexisting shape limits are explicit in the test.
...
const inputs = [
  ...
  [
    "mml-mi",
    "\\mathbf{\\mmlToken{mi}{x}}"
  ],
  [
    "mml-mn",
    "\\mathbf{\\mmlToken{mn}{12}}"
  ],
  [
    "mml-mo",
    "\\mathbf{\\mmlToken{mo}{+}}"
  ...
  [
    "mml-kept",
    "\\mathbf{\\mmlToken{mi}[mathvariant=double-struck]{R}}"
  ],
  [
    "declaration-policy",
    "{\\bf x+12+\\alpha+\\Gamma+\\mmlToken{mi}{z}}+y"
  ],
  [
    "argument-policy",
    "\\mathbf{\\frac{\\alpha+x}{\\Gamma+12}}"
  ],
  [
    "group-policy",
    "\\mathbf{{\\alpha+x}+\\Gamma}"
  ],
  [
    "plain-control",
    "\\alpha+\\Gamma+\\mmlToken{mi}{x}"
  ],
  [
    "colon-close",
    "\\mathbf{x:y)}"
  ...
  [
    "fixed-nested",
    "\\mathrm{\\mathbf{\\alpha+\\Gamma+x}}"
  ],
  [
    "fixed-nested-reverse",
    "\\mathbf{\\mathrm{\\alpha+\\Gamma+x}}"
  ],
  [
    "mml-nested",
    "\\mathbf{\\mathrm{\\mmlToken{mi}{x}+y}}"
  ],
  [
    "mml-nested-kept",
    "\\mathrm{\\mathbf{\\mmlToken{mi}[mathvariant=double-struck]{R}+x}}"
  ],
  [
    "fixed-declarations",
    "{\\bf \\alpha+\\Gamma{\\rm \\alpha+\\Gamma}+x}+y"
  ],
  [
    "accent-dot",
    "\\mathbf{\\dot{x}}"
...
fs.writeFileSync(path.join(__dirname,'font_token_policy_mathjax_3_2_2.json'),JSON.stringify({oracle:'Unmodified pinned D2 MathJax3.2.2; fresh VM per case/mode; AST observed before unchanged SVG renderer',mathjaxGitCommit:'ad8f5c21cb810236551da8c6512ba733e67357ee',assetsSHA256:hashes,cases:records},null,2)+'\n');console.log(records.length+' exact AST and complete SVG cases');
