...
Commutator, anticommutator and Poisson-bracket aliases parse the original combined argument expression in a child parser. This preserves punctuation spacing, font scope and shared configuration, while honoring the original delimiter mapping, star/size modifiers and argument requirements.
...
Declared paired delimiters use source-safe argument substitution and continue in the caller after joining the remaining input. Explicit sizes retain their supplied command, and one macro expansion is charged after installing the joined source. Five retained examples assert complete original SVGs, with two display measurements across the corpus and three original source/cursor/count traces. Two additional slash examples retain their original SVGs but separately assert exact unchanged baseline output: the missing ORD wrapper belongs to the later cleanStretchy filter and is not fixed here. Existing XPP and macro-limit controls remain in force.

Equation environments preserve the original per-parser nesting guard, including across captured table bodies and source continuations. Genuine child parsers have independent nesting state. Eight retained public references compare complete original SVGs, with four measurable nesting-error outputs; percentage-width single-equation controls are not asserted to have fixed pixel dimensions. The two historical inline sequential-equation controls now select their original error SVGs.
